...
#define LOGIC_2_RAW(i) (i - 1)
      ...
      -> ChfsResult<inode_id_t>
  {
         ...
         iter.next(bm->block_size()).unwrap(), count++)
    {
      ...
      if (free_idx)
      {
        ...
        Inode inode(type, bm->block_size());                                        // create a new inode with the given type
        inode_id_t inode_id = free_idx.value() + count * 8 * bm->block_size();      // get the raw inode id
        u64 inode_table_page = inode_id / (bm->block_size() / sizeof(inode_id_t));  // which page the inode table is in
        u64 inode_table_index = inode_id % (bm->block_size() / sizeof(inode_id_t)); // which index in the page
        // write the block id to the inode table
        bm->write_partial_block(1 + inode_table_page, reinterpret_cast<u8 *>(&bid), inode_table_index * sizeof(block_id_t), sizeof(block_id_t));
        std::vector<u8> inode_buffer(bm->block_size());
        inode.flush_to_buffer(inode_buffer.data()); // write the inode to the buffer
        bm->write_block(bid, inode_buffer.data());  // write the inode to the block

        return (ChfsResult<inode_id_t>(RAW_2_LOGIC(inode_id))); // use RAW_2_LOGIC to get the logic inode id
      }
    }
  ...
  // { Your code here }
  auto InodeManager::set_table(inode_id_t inode_id, block_id_t bid) -> ChfsNullResult
  {

    // TODO: Implement this function.
    // Fill `bid` into the inode table entry
    // whose index is `idx`.
    u64 inode_table_page = inode_id / (bm->block_size() / sizeof(inode_id_t));
    u64 inode_table_index = inode_id % (bm->block_size() / sizeof(inode_id_t));
    bm->write_partial_block(1 + inode_table_page, reinterpret_cast<u8 *>(&bid), inode_table_index * sizeof(block_id_t), sizeof(block_id_t));
  ...
  auto InodeManager::get(inode_id_t id) -> ChfsResult<block_id_t>
  {
    ...
    // table index.
    inode_id_t inode_id = LOGIC_2_RAW(id); // use LOGIC_2_RAW to get the raw inode id
    u64 inode_table_page = inode_id / (bm->block_size() / sizeof(inode_id_t));
    u64 inode_table_index = inode_id % (bm->block_size() / sizeof(inode_id_t));
    std::vector<inode_id_t> inode_table(bm->block_size() / sizeof(inode_id_t));       // create a vector to store the page which contains the inode table
    bm->read_block(1 + inode_table_page, reinterpret_cast<u8 *>(inode_table.data())); // read the inode table to the buffer

    res_block_id = inode_table[inode_table_index];
  ...
  auto InodeManager::free_inode(inode_id_t id) -> ChfsNullResult
  {
    ...
    block_id_t bid = 0;                    // create a block id to clear the inode table entry
    inode_id_t inode_id = LOGIC_2_RAW(id); // use LOGIC_2_RAW to get the raw inode id
    u64 inode_table_page = inode_id / (bm->block_size() / sizeof(inode_id_t));
    u64 inode_table_index = inode_id % (bm->block_size() / sizeof(inode_id_t));
    // write the zero block id to the inode table entry
    bm->write_partial_block(1 + inode_table_page, reinterpret_cast<u8 *>(&bid), inode_table_index * sizeof(block_id_t), sizeof(block_id_t));

    u64 inode_bitmap_page = inode_id / bm->block_size();
    u64 inode_bitmap_index = inode_id % bm->block_size();
    std::vector<u8> bitmap_buffer(bm->block_size());
    bm->read_block(1 + n_table_blocks + inode_bitmap_page, bitmap_buffer.data());  // read the inode bitmap to the buffer
    Bitmap bitmap(bitmap_buffer.data(), bm->block_size());                         // create a bitmap object from the buffer
    bitmap.clear(inode_bitmap_index);                                              // clear the inode bitmap
    bm->write_block(1 + n_table_blocks + inode_bitmap_page, bitmap_buffer.data()); // write the inode bitmap back to the block
