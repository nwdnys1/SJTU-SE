...
namespace chfs
{
  ...
  public:
    ...
    // add functions for block operations:
    ChfsNullResult add_blockinfo(inode_id_t inode_id, BlockInfo &block_info);
    ChfsNullResult remove_blockinfo(inode_id_t inode_id, block_id_t block_id, mac_id_t machine_id);
    ChfsNullResult get_blockinfo_list(inode_id_t inode_id, std::vector<BlockInfo> &res);
    ChfsNullResult unlink_block(inode_id_t inode_id, const std::string &file_name);
