...
namespace chfs
{
      ...
      : file_name_(file), block_cnt(block_cnt), in_memory(false)
  {
    ...
    auto file_sz = get_file_sz(this->file_name_);
    if (file_sz == 0) // if the file is empty, we need to initialize it
      initialize_file(this->fd, this->total_storage_sz());
    else // if the file is not empty, we need to check the size
    {
      this->block_cnt = file_sz / this->block_sz;
      ...
      -> ChfsNullResult
  {
    ...
    u64 block_offset = block_id * this->block_sz;

    if (this->in_memory) // if in memory, we can directly copy the data using memcpy
      memcpy(this->block_data + block_offset, data, this->block_sz);
    else // if not in memory, we need to write the data to the file
    {
      if (lseek(this->fd, block_offset, SEEK_SET) == -1)
      ...
      -> ChfsNullResult
  {
    ...
    u64 block_offset = block_id * this->block_sz + offset;
  ...
  auto BlockManager::zero_block(block_id_t block_id) -> ChfsNullResult
  {
    ...
    if (this->in_memory)
      memcpy(this->block_data + block_offset, zero_block, this->block_sz);
    else
    {
      if (lseek(this->fd, block_offset, SEEK_SET) == -1)
        return ChfsNullResult(ErrorType::DONE);
    ...
    // remember to delete the zero block
    delete[] zero_block;
