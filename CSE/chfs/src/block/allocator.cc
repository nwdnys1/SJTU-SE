...
namespace chfs
{
  ...
  auto BlockAllocator::allocate() -> ChfsResult<block_id_t>
  {
    ...
    for (uint i = 0; i < this->bitmap_block_cnt; i++)
    {
      ...
      Bitmap bitmap(buffer.data(), bm->block_size()); // create a bitmap object from the bitmap block
      ...
      if (i == this->bitmap_block_cnt - 1)
      {
        ...
        std::optional<usize> obj =
            bitmap.find_first_free_w_bound(this->last_block_num); // since the last block may not be full, need a upper bound
        if (obj.has_value())
          res = obj.value();
      }
      else
      ...
      if (res)
      {
        ...
        bitmap.set(res.value());                                   // set the bit to 1
        bm->write_block(i + this->bitmap_block_id, buffer.data()); // write the bitmap back to the bitmap block
        retval = res.value() + 8 * i * bm->block_size();           // calculate the block id
  ...
  auto BlockAllocator::deallocate(block_id_t block_id) -> ChfsNullResult
  {
    ...
    std::vector<u8> buffer(bm->block_size());
    u64 bitmap_block_offset = block_id / (8 * bm->block_size()); // 第几个bitmap block
    u64 bitmap_bit_index = block_id % (8 * bm->block_size());    // 在bitmap block中的第几位

    bm->read_block(bitmap_block_offset + this->bitmap_block_id, buffer.data()); // 读出blockid所在的bitmap block
    Bitmap bitmap(buffer.data(), bm->block_size());                             // 创建bitmap对象
    if (!bitmap.check(bitmap_bit_index))                                        // 检查是否合法
      return ChfsNullResult(ErrorType::INVALID_ARG);

    bitmap.clear(bitmap_bit_index);                                              // 清零
    bm->write_block(bitmap_block_offset + this->bitmap_block_id, buffer.data()); // 把bitmap写回block manager
