...
stat_t nexti(y64sim_t *sim)
{
    ...
    if ((icode >= 2 && icode <= 6)|| icode == 0xB || icode == 0xA) {
        byte_t regAB = 0;
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        if (NONE_REG(regA) || NONE_REG(regB)) {
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        ...
        if (!set_long_val(sim->m, valC+valB, get_reg_val(sim->r, regA))) {
        	err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, valC+valB);
        	return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        ...
        valB = get_reg_val(sim->r, regB);
        if (!get_long_val(sim->m, valC+valB, &valC)) {
        	err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, valC+valB);
        	return STAT_ADR;
        }
        set_reg_val(sim->r, regA, valC);
        sim->pc = next_pc;
        break;
      ...
      case I_JMP: /* 7:x imm */
        if(ifun < 0 || ifun > 6) {
        	err_print("PC = 0x%lx, Invalid condition code", sim->pc);
        	return STAT_INS;
        }
        if (cond_doit(sim->cc, ifun)) {
            sim->pc = valC;
      ...
      case I_CALL: /* 8:x imm */
        ...
        break;
      case I_RET: /* 9:0 */
        if (!get_long_val(sim->m, get_reg_val(sim->r, REG_RSP), &valC)) {
        	err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, get_reg_val(sim->r, REG_RSP));
        	return STAT_ADR;
        }
        set_reg_val(sim->r, REG_RSP, get_reg_val(sim->r, REG_RSP)+8);
      ...
      case I_PUSHQ: /* A:0 regA:F */
        ...
        if (!set_long_val(sim->m, get_reg_val(sim->r, REG_RSP), get_reg_val(sim->r, regA))) {
        	err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc, get_reg_val(sim->r, REG_RSP));
        	return STAT_ADR;
        }
        if(regA == REG_RSP) {//if regA is %rsp, then the value of %rsp should be updated
        	set_long_val(sim->m, get_reg_val(sim->r, REG_RSP), get_reg_val(sim->r, regA)+8);
        }
        sim->pc = next_pc;
        break;
