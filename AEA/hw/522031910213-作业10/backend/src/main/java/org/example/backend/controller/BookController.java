...
public class BookController {
	...
	@GetMapping("/search")
	public Result<PageResponse<Book>> searchBooks(@RequestParam(defaultValue = "") String keyword,
			@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int pageSize) {
		return service.searchBooks(keyword, page, pageSize);
	}
	@GetMapping("/search/by_tag")
	public Result<PageResponse<Book>> searchBooksByTagIds(@RequestParam(defaultValue = "") List<Long> tid,
			@RequestParam(defaultValue = "1") int page, @RequestParam(defaultValue = "10") int pageSize) {
		return service.searchBookByTagIds(tid, page, pageSize);
	}
	@PostMapping("admin/cover/{id}")
	...
	public Result<List<SalesInfo>> rank(
