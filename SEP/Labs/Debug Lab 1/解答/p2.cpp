...
Board::Board(int num_disk) : num_disk(num_disk) {
    ...
    disks = new const Disk*[num_disk];//添加对disks的初始化
...
void Board::move(int from, int to, bool log) {
    ...
    if (from < 0 || from > 2 || to < 0 || to > 2)
        return;
    if (rods[from]->empty()) return;//添加特判
    if (!rods[to]->empty() && rods[from]->top()->val > rods[to]->top()->val)//添加非空条件
        return;
    if (rods[to]->full())
        return;
...
void solve(int n, int src, int buf, int dest, Queue<pair<int,int>> &solution) {//添加引用
    if (n >= 1) {
