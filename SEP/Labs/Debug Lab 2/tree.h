...
public:
  ...
  void remove_from_leaf(int key, node_t *node) {
    ...
    if (node->up) {
      ...
      if (index_in_parent) {
          if (node->key_list.size()) node->up->key_list[index_in_parent - 1] = node->key_list.front();
          else {
              if (node->right) node->up->key_list[index_in_parent - 1] = node->right->key_list.front();
              else {}
          }
      }
    }
